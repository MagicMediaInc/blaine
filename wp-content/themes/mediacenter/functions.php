...
if ( !class_exists( 'ReduxFramework' ) && file_exists( get_template_directory() . '/settings/redux/ReduxCore/framework.php' ) ) {
    require_once( get_template_directory() . '/settings/redux/ReduxCore/framework.php' );
}
if ( !isset( $redux_demo ) && file_exists( get_template_directory() . '/settings/mediacenter.config.php' ) ) {
...
function register_mc_widgets() { 
	if ( class_exists( 'WC_Widget' ) ) {
		wc_get_template( 'framework/widgets/class-mc-widget-home-tabs.php');
		wc_get_template( 'framework/widgets/class-mc-widget-brands-filter.php');
		wc_get_template( 'framework/widgets/class-mc-widget-vertical-menu.php');
	}
...
function media_center_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> 'http://downloads.wordpress.org/plugin/woocommerce.2.2.8.zip', // The plugin source
		...
		array(
			...
			'source'   				=> 'http://downloads.wordpress.org/plugin/yith-woocommerce-compare.1.2.1.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
		...
		array(
			...
			'source'   				=> 'http://downloads.wordpress.org/plugin/yith-woocommerce-wishlist.1.1.6.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
        ...
        array(
			...
			'source'   				=> 'http://downloads.wordpress.org/plugin/regenerate-thumbnails.2.2.4.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
...
function media_center_scripts(){
	...
	*/
	...
	wp_enqueue_script( 'media_center-bootstrap', get_template_directory_uri() . '/assets/js/bootstrap.js', array( 'jquery' ), '1.10.2', true );
...
function media_center_font_url() {
	...
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'media_center' ) ) {
		$font_url = add_query_arg( 'family', urlencode( 'Open Sans:400,600,700,800' ), "//fonts.googleapis.com/css" );
	}
...
function media_center_wp_title( $title, $sep ) {
	...
	if( is_int($paged) && is_int($page) ){
		if ( $paged >= 2 || $page >= 2 ){
			$title = "$title $sep " . sprintf( __( 'Page %s', 'reen' ), max( $paged, $page ) );
		}
	}
...
if ( class_exists( 'WooCommerce' ) ) {
	wc_get_template( 'framework/inc/mediacenter-breadcrumb.php' );
}
