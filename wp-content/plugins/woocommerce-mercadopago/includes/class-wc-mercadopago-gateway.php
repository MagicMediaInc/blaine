...
class WC_MercadoPago_Gateway extends WC_Payment_Gateway {
	...
	public function generate_form( $order_id ) {
		...
		if ( $url ) {
			...
			$html .= '<div class="row">';
			$html .= '<div class="col-xs-12 col-sm-6"><a class="btn-block le-button big checkout-button button alt wc-forward" id="submit-payment" href="' . $url . '" name="MP-Checkout" class="button alt" mp-mode="modal">' . __( 'Pay via MercadoPago', 'woocommerce-mercadopago' ) . '</a></div> <div class="col-xs-12 col-sm-6"><a class="button cancel btn-block le-button big inverse button" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . __( 'Cancel order &amp; restore cart', 'woocommerce-mercadopago' ) . '</a></div>';
			$html .= '</div>';
			// Add MercadoPago JS.
