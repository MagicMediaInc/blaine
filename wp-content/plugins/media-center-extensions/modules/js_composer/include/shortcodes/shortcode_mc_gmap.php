...
function shortcode_mc_gmap( $atts, $content = null ){
	...
	if( $display_get_directions_form ) :
		...
		$gmapHTML .= "\t\t\t\t\t\t" . '<input type="text" id="starting-point" class="le-input input-lg form-control" placeholder="Enter Your Starting Point">' ;
		$gmapHTML .= "\t\t\t\t\t\t" . '<span class="input-group-btn">' ;
		$gmapHTML .= "\t\t\t\t\t\t\t" . '<button id="get-direction" class="btn btn-lg le-button" type="button">Get Directions</button>' ;
		$gmapHTML .= "\t\t\t\t\t\t" . '</span>' ;
